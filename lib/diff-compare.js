var diffCompare = module.exports = {
   ...
   *
   * - base: the string that will be used as the base input to SequenceMatcher
   * - nextText: the string that will be used as the new text input to SequenceMatcher
   *
   * or
   *
   * - base: the array of strings that was used as the base text input to SequenceMatcher
   * - compare: the array of strings that was used as the new text input to SequenceMatcher
  ...
  build: function (params) {
    var base = typeof params.base === 'string' ?
      diffCompare.stringAsLines(params.base) :
      params.base
    var compare = typeof params.compare === 'string' ?
      diffCompare.stringAsLines(params.compare) :
      params.compare
    var opcodes = params.opcodes === undefined ?
      (new diffCompare.SequenceMatcher(base, compare)).get_opcodes() :
      params.opcodes
    var contextSize = params.contextSize || 999

    if (base === null) {
      throw new Error('Cannot build diff view base is not defined.')
    }
    if (compare === null) {
      throw new Error('Cannot build diff view compare is not defined.')
    }
    if (!opcodes) {
    ...
    for (var idx = 0; idx < opcodes.length; idx++) {
      ...
      for (i = 0; i < rowcnt; i++) {
        ...
        toprows.push(node = [])
        b = addCells(node, b, be, base, change)
        n = addCells(node, n, ne, compare, change)
      }
